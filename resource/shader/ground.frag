...
void main() {
    ...
    float dist_to_cam = length(viewspace_position);
    float fog_factor = clamp(pow(dist_to_cam / 180, 2), 0, 1);
    out_color = vec4(out_rgb * min(1/fog_factor, 1) + vec3(0,0.25,0.5) * fog_factor, 1);
